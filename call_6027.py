...
#   f(3, B) = f(2, f(3, B-1)) cant be simplified. also note that the recursive branch goes from B to 0
...
def worker(start, end):
    for r7 in range(start, end):
        ...
        f3[0] = (
            r7 * (r7 + 1) + 2 * r7 + 1
        ) % 32768  # start with f(3, 0) = f(2, r7) from above eqs
...
def main_single():
    ...
    for r7 in range(0, 32768):
        ...
        if diff > 1:
            ...
            print(
                'r7 = {}, {:.2f} it/sec, {:.2f} min left'.format(
                    r7, speed, eta
                )
            )
            last_val = r7
